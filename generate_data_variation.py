...
def generate_batch(count, samples, feature_dim, device=None, **kwargs):
    ...
    for _ in tqdm(range(count), desc="Generating batch"):
        X, Xa=generate_dataset(samples, feature_dim, device=device, **kwargs)
        batch_X.append(X)
        batch_Xa.append(Xa)

    return torch.stack(batch_X), torch.stack(batch_Xa)
...
if True and __name__=="__main__":
    ...
    X, Xa =generate_dataset(N,10, device=device)
    
    t1=time()
    print("Required time", t1-t0)
    #import numpy as np
    #print(np.unique(Z.numpy(), return_counts=True))
